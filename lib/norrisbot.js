...
NorrisBot.prototype._onMessage = function (message) {
    if (this._isChatMessage(message) &&
        (this._isPublicChannelConversation(message) ||
        this._isPrivateChannelConversation(message) ||
        this._isDirectMessageConversation(message)) &&
...
NorrisBot.prototype._replyWithRandomJoke = function (originalMessage) {
    ...
    self.db.get('SELECT id, joke FROM jokes ORDER BY used ASC, RANDOM() LIMIT 1', function (err, record) {
        ...
        if(self._isPublicChannelConversation(originalMessage)) {
            console.log('bot replying to public channel');
            var channel = self._getChannelById(originalMessage.channel);
            self.postMessageToChannel(channel.name, record.joke, {as_user: true});
            self.db.run('UPDATE jokes SET used = used + 1 WHERE id = ?', record.id);
        }else if(self._isPrivateChannelConversation(originalMessage)) {
            console.log('bot replying to private group');
            var group = self._getGroupById(originalMessage.channel);
            console.log(group)
            self.postMessageToGroup(group.name, record.joke, {as_user: true});
            self.db.run('UPDATE jokes SET used = used + 1 WHERE id = ?', record.id);
        }else{
            console.log('bot replying to direct message');
            var user = self._getUserById(originalMessage.user);
            self.postMessageToUser(user.name, record.joke, {as_user: true});
            self.db.run('UPDATE jokes SET used = used + 1 WHERE id = ?', record.id);
        }
    });
};
...
NorrisBot.prototype._isChatMessage = function (message) {
    console.log(message);
    var isChatMsg = message.type === 'message' && Boolean(message.text);
    return isChatMsg;
};

/**
 * Util function to check if a given real time message object is directed to a public channel
 * @param {object} message
 * @returns {boolean}
 * @private
 */
NorrisBot.prototype._isPublicChannelConversation = function (message) {
    var isPublicChannel = typeof message.channel === 'string' &&
        message.channel[0] === 'C';
    return isPublicChannel;
};

/**
 * Util function to check if a given real time message object is directed to a private channel
 * @param {object} message
 * @returns {boolean}
 * @private
 */
NorrisBot.prototype._isPrivateChannelConversation = function (message) {
    var isPrivateChannel = typeof message.channel === 'string' &&
        message.channel[0] === 'G';
    return isPrivateChannel;
};

/**
 * Util function to check if a given real time message object is directed to a direct message conversation
 * @param {object} message
 * @returns {boolean}
 * @private
 */
NorrisBot.prototype._isDirectMessageConversation = function (message) {
    var isPrivateChannel = typeof message.channel === 'string' &&
        message.channel[0] === 'D';
    return isPrivateChannel;
};
...
/**
 * Util function to get the name of a group given its id
